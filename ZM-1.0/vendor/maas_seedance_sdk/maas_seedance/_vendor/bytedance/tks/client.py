...
import requests
from typing_extensions import IO, TYPE_CHECKING, Any, Dict, Optional, Union
from volcenginesdkcore.rest import ApiException
from bytedance.jeddak_secure_channel.ra import verify_ra_result, gen_report_data
...
PATH_RAS_VERIFY = "/api/ras/v1/verify"
...
class TKSClient:
    ...
    def get_key(
        ...
        # 获取TKS的证明报告
        ra_result = self._do_server_ra(nonce=nonce, bi_auth=True)
        # 调用verify接口对TKS进行远程证明
        if policy_id:
            if self._config.top_config:
                url = self._config.addr
                self._config.top_config["action"] = "VerifyRasEvidence"
            else:
                url = self._get_tks_url(PATH_RAS_VERIFY)
            report_data_bytes = gen_report_data(nonce, ra_result["Challenge"]["NonceDown"], ra_result["DHParam"],
                                                ra_result['Challenge']['ExpirationTime'])

            urlsafe_base64 = base64.urlsafe_b64encode(report_data_bytes)
            report_data = urlsafe_base64.rstrip(b'=').decode('utf-8')
            req_body = {
                "Evidence": ra_result.get("Report").rstrip('='),
                "PolicyId": policy_id,
            ...
            verify_res = verify_ra_result(res.get("AttestationResult"), None, "", None, nonce,
                                          report_data_bytes.hex())
            if not verify_res:
...
if __name__ == "__main__":
    ...
    req_body = {"Username": "alice", "Password": md5_hash("alice0".encode(), ts)}
    response = requests.post(
        "https://jeddakchain.bytedance.com/api/user/login",
    ...
    cfg = TKSConfig(addr="jeddakchain.bytedance.com", enable_tls=True)
    client = TKSClient("alice", cfg)
    # print(
    #     "Key Get",
    #     client.get_key(
    #         "2ee6348f-fafa-4e97-a824-0b6ed0e4d07e", "f532b84c-8f80-4705-b5a7-6217f4729924"
    #     ),
    # )
    #
    # print("Challenge Server", client.challenge_server("1234"))
    # print(
    #     "Key Creation",
    #     client.create_key(
    #         "4894a298-759a-4b94-a292-f43b2f8deea6",
    #         "SYMMETRIC_256",
    #         "test-encrypt-sdk",
    #         desc="测试本地加密SDK",
    #         cookies=cookies,
    ...
    # print("Key Set Policy", client.set_key_policy(
    #     "b3bf3cd7-2a68-427f-865f-8688dc96c7d8",
    #     "1110fb2f-8dca-4e27-9a37-04a60585fd29",
    #     json.dumps([
    #         [
    #             {
    #                 "version": "v1",
    ...
    #                 "optional": False,
    #                 "inversion": False,
    #                 "policyID": "78485ba8-4e67-46a5-a34e-1a9b7930df02"
    #             }
    #         ]
    #     ])
    # ))
