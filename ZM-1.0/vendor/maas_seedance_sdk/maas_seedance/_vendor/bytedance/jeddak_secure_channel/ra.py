...
def attest_server_local(token: Optional[str], config: RaConfig, nonce=None):
    """
    对数据接收方进行远程证明(非TCA方式).
    """
...
def attest_server(token: Optional[str], config: RaConfig, nonce: str = None) -> Tuple[bool, str]:
    ...
    try:
        if config.bytedance_top_info:
            ...
            response = attest_server_local(token, config, nonce)
    ...
    if isinstance(response, requests.Response):
        if response.status_code != 200:
            ...
            if not ra_res:
                ra_res = json.loads(response_text).get("ResponseResult")
    else:
        ra_res = response
        ...
        try:
            if challenge_report_data:
                ...
                if not nonce_ret:
                    ...
                    # return False
                    logger.debug("verify nonce failed")
                    return True
